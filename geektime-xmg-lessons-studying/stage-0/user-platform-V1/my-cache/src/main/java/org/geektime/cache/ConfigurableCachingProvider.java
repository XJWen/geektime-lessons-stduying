...
public class ConfigurableCachingProvider implements CachingProvider {
    ...
    private String generateCacheManagerKey(URI actualURI, ClassLoader actualClassLoader, Properties actualProperties) {
        StringBuilder keyBuilder = new StringBuilder(actualURI.toASCIIString())
                .append("-").append(actualClassLoader)
                .append("-").append(actualProperties);

        return keyBuilder.toString();
    }

    private CacheManager newCacheManager(URI actualURI, ClassLoader actualClassLoader, Properties actualProperties) {
        CacheManager cacheManager = null;
        try {
            Class<? extends AbstractCacheManager>  cacheManagerClass = getCacheManagerClass(actualURI, actualClassLoader, actualProperties);
            ...
            cacheManager = constructor.newInstance(this,actualURI,actualClassLoader,actualProperties);
        }catch (Throwable ex){
    ...
    private Class<? extends AbstractCacheManager> getCacheManagerClass
            (URI actualURI, ClassLoader actualClassLoader, Properties actualProperties) throws  ClassNotFoundException,ClassCastException{
        String className = getCacheManagerClassName(actualURI,actualProperties);
        Class<? extends AbstractCacheManager> cacheManagerImplClass = null;
        Class<?> cacheManagerClass = actualClassLoader.loadClass(className);
    ...
    private String getCacheManagerClassName(URI actualURI, Properties actualProperties) {
        String propertyName = getCacheManagerClassNamePropertyName(actualURI);
        String className = actualProperties.getProperty(propertyName);
        if (className == null){
            throw new IllegalStateException(format(
               "The implementation class name of %s that is the value of property '%s' must be configured in the Properties[%s]",
               CacheManager.class.getName(),propertyName,actualProperties
            ));
        }
        return className;
    }

    private String getCacheManagerClassNamePropertyName(URI actualURI) {
